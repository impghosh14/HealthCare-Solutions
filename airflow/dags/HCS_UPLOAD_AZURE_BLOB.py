...
from src.blob_utils.upload_azure import upload_file as upload_azure_file
...
def run_upload(local_path: str = None, container_path: str = None, container_blob_dir: str = None, **kwargs):
    """
    Resolution: dag_run.conf > payload file > op_kwargs
    Expected payload keys:
      - local_path (path inside container to the file)
      - container_blob_dir (destination directory/container path in blob container; optional)
      - container_name (azure container name) optional (fallback to .env)
    """
    dag_run = kwargs.get("dag_run")
    conf = (dag_run.conf if dag_run else {}) or {}
    dag_id = kwargs.get("dag").dag_id if kwargs.get("dag") else "HCS_UPLOAD_AZURE_BLOB"
    ...
    final_blob_dir = conf.get("blob_dir") or payload.get("blob_dir") or container_blob_dir or ""
    final_container = conf.get("container_name") or payload.get("container_name") or None

    if not final_local:
        raise AirflowFailException("Missing required 'local_path' (path inside container).")

    try:
        uri = upload_azure_file(local_path=final_local, container_name=final_container, blob_dir=final_blob_dir)
        kwargs["ti"].xcom_push(key="azure_blob_uri", value=uri)
...
with DAG(
    dag_id="HCS_UPLOAD_AZURE_BLOB",
    default_args=DEFAULT_ARGS,
    schedule_interval=None,
    start_date=datetime(2025, 1, 1),
    catchup=False,
    max_active_runs=1,
    tags=["HCS", "upload", "azure"]
) as dag:
